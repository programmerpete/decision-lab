...
| Jev access                  | Owner reports access with OpenRouter. Confirm exact account listing, model ID, transport and credential type.                   |
| Documented direct Jev route | TypeSafe `/v1/systemone`; verify current request shape from the official quickstart. Requires appropriate TypeSafe credentials. |
| Backend host                | Not provisioned. Choose a small server/serverless host after verifying runtime compatibility.                                   |
...
1. Make one manually verified synthetic request per provider from a local server; do not commit response secrets.
